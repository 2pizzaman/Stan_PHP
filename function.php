...
/* 4.arrChuk
$arr=array(13,14,15,12,11,7,5,4,3,123,145,345);
function arr_chuk($arr, $part){
    $arr_size=round(count($arr)/$part);
    $arr_piece=[];
    $piece=0;
    //   foreach ($arr as $key=>$value){
    for($key=0;$key<$arr_size; $key++){
        if(isset($arr[$key])){
            for($j=0;$j<$part;$j++){
                if(isset($arr[$piece])){
                    $arr_piece[$key][]=$arr[$piece];
                    $piece++;
                }
            }
            //         }
...
arr_chuk($arr, 3);*/
//echo "Разбитый на.' $part'1. частей: <br>\n";
